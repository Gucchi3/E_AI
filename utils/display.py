...
def print_training_info(config: AppConfig, model: torch.nn.Module, device: torch.device, run_dir: Path, macs: str) -> None:
    ...
    if config.model.name in QUANTIZED_MODEL_NAMES:
        ...
        if config.model.name in {"basic_vit_int8", "basic_vit_int4", "basic_vit_fp4", "basic_vit_ufp4", "basic_vit_test2", "basic_vit_test5", "basic_vit_test6", "basic_vit_test7", "basic_vit_test8", "basic_vit_test9"}:
...
def _quantization_formats(config: AppConfig) -> tuple[str, str]:
    ...
    if config.model.name == "basic_vit_test8":
        return "INT8 (first/classifier), FP4 E2M1 (body)", "UINT4 (after ReLU), FP4 E2M1 (signed attention), shared INT8 (residual), INT8 (average pool)"
    if config.model.name == "basic_vit_test9":
        return "INT8 (first/classifier), FP4 E2M1 (body)", "UFP4 E2M2 (after ReLU), FP4 E2M1 (signed attention), shared INT4 (residual), INT8 (average pool)"
    ...
    if config.model.name == "int4_cnn":
        return "INT8 (first/classifier), signed INT4 (body)", "UINT4 (after ReLU), UINT8 (before classifier)"
