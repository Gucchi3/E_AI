"""Attention経路をINT8、残差を独立INT8 scaleにしたBasicViTのUFP4テストモデル。"""
...
from utils.quantization import FP4Quantizer, IntegerQuantizer, PerTokenNEQ, QuantConv2d, QuantLinear, QuantMatMul, QuantResidualAdd, QuantizedAvgPool2d, UFP4Quantizer


class Test3BasicViTStem(nn.Module):
...
class Test3ConvolutionalFeedForward(nn.Module):
    """FP4重み・UFP4活性化とINT8 projection出力を使用する畳み込みFFN。"""
...
class Test3LocalFeedForwardBlock(nn.Module):
    """このblock専用のINT8 scaleで残差加算するローカルブロック。"""

    def __init__(self, channels: int, hidden_channels: int, rounding: str, activation_range_momentum: float) -> None:
        super().__init__()
        self.ffn          = Test3ConvolutionalFeedForward(channels, hidden_channels, rounding, activation_range_momentum)
        self.residual_add = QuantResidualAdd(bit_width=8, rounding=rounding, range_momentum=activation_range_momentum)

    @property
    def output_scale(self) -> torch.Tensor:
        """このblockの残差加算後に得られるINT8 scaleを返す。"""
...
class Test3SimpleAttention(nn.Module):
    ...
    @property
    def output_scale(self) -> torch.Tensor:
        """Attention主枝のprojection後に得られるINT8 scaleを返す。"""
        return self.project_quantizer.scale
...
class Test3AttentionFeedForwardBlock(nn.Module):
    """INT8 AttentionとUFP4 FFNを別々のINT8 scale残差で連結する。"""
    ...
    def __init__(self, channels: int, hidden_channels: int, num_heads: int, key_dim: int, value_dim: int, rounding: str, activation_range_momentum: float) -> None:
        super().__init__()
        self.attention              = Test3SimpleAttention(channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
...
class BasicViTTest3(nn.Module):
    """INT8 Attentionと6個の独立INT8残差scaleを持つUFP4モデル。"""
    ...
    def __init__(self, num_classes: int = 10, input_bits: int = 8, residual_bits: int = 8, rounding: str = "ties_away_from_zero", activation_range_momentum: float = 0.95, image_size: int = 32, embed_dim: int = 48, mlp_ratio: int = 2, num_heads: int = 2, key_dim: int = 12, value_dim: int = 24) -> None:
        super().__init__()
        if image_size != 32:
            raise ValueError("BasicViTTest3 is designed specifically for 32x32 images.")
        if input_bits != 8:
            raise ValueError("BasicViTTest3 requires input_bits=8.")
        if residual_bits != 8:
            raise ValueError("BasicViTTest3 requires residual_bits=8.")
        ...
        self.stem                       = Test3BasicViTStem(embed_dim, rounding, activation_range_momentum)
        self.stem_output_quantizer      = IntegerQuantizer(bit_width=8, signed=True, rounding=rounding, range_momentum=activation_range_momentum)
        self.local_block1               = Test3LocalFeedForwardBlock(embed_dim, hidden_channels, rounding, activation_range_momentum)
        self.local_block2               = Test3LocalFeedForwardBlock(embed_dim, hidden_channels, rounding, activation_range_momentum)
        self.attention_block1           = Test3AttentionFeedForwardBlock(embed_dim, hidden_channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
        self.attention_block2           = Test3AttentionFeedForwardBlock(embed_dim, hidden_channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
