"""BasicViT with signed INT4 body activations, signed INT4 body weights, and a shared INT8 residual stream."""
...
class SignedINT4BasicViTStem(nn.Module):
    """Keep the image-facing convolution in INT8 and use signed INT4 inside the stem."""
    ...
    def __init__(self, embed_dim: int, rounding: str, activation_range_momentum: float) -> None:
        ...
        self.quantizer1 = IntegerQuantizer(bit_width=4, signed=True, rounding=rounding, range_momentum=activation_range_momentum)
        ...
        self.quantizer2_depthwise = IntegerQuantizer(bit_width=4, signed=True, rounding=rounding, range_momentum=activation_range_momentum)
...
class SignedINT4ConvolutionalFeedForward(nn.Module):
    """Convolutional feed-forward network using signed INT4 activations and weights."""
    ...
    def __init__(self, channels: int, hidden_channels: int, rounding: str, activation_range_momentum: float) -> None:
        ...
        self.expand_quantizer = IntegerQuantizer(bit_width=4, signed=True, rounding=rounding, range_momentum=activation_range_momentum)
        ...
        self.depthwise_quantizer = IntegerQuantizer(bit_width=4, signed=True, rounding=rounding, range_momentum=activation_range_momentum)
...
class SignedINT4LocalFeedForwardBlock(nn.Module):
    """Local feed-forward block connected to the shared INT8 residual stream."""

    def __init__(self, channels: int, hidden_channels: int, rounding: str, activation_range_momentum: float) -> None:
        super().__init__()
        self.ffn          = SignedINT4ConvolutionalFeedForward(channels, hidden_channels, rounding, activation_range_momentum)
...
class SignedINT4SimpleAttention(nn.Module):
    """Softmax-free attention with signed INT4 NEQ, attention maps, and matrix multiplication."""
    ...
    def __init__(self, channels: int, num_heads: int, key_dim: int, value_dim: int, rounding: str, activation_range_momentum: float) -> None:
        ...
        self.project_input_quantizer = IntegerQuantizer(bit_width=4, signed=True, rounding=rounding, range_momentum=activation_range_momentum)
...
class SignedINT4AttentionFeedForwardBlock(nn.Module):
    ...
    def __init__(self, channels: int, hidden_channels: int, num_heads: int, key_dim: int, value_dim: int, rounding: str, activation_range_momentum: float) -> None:
        super().__init__()
        self.attention              = SignedINT4SimpleAttention(channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
...
class BasicViTTest6(nn.Module):
    """Test 6: signed INT4 body activations and weights with a globally shared INT8 residual scale."""
    ...
    def __init__(self, num_classes: int = 10, input_bits: int = 8, residual_bits: int = 8, rounding: str = "ties_away_from_zero", activation_range_momentum: float = 0.95, image_size: int = 32, embed_dim: int = 48, mlp_ratio: int = 2, num_heads: int = 2, key_dim: int = 12, value_dim: int = 24) -> None:
        super().__init__()
        if image_size != 32:
            raise ValueError("BasicViTTest6 is designed specifically for 32x32 images.")
        if input_bits != 8:
            raise ValueError("BasicViTTest6 requires input_bits=8.")
        if residual_bits != 8:
            raise ValueError("BasicViTTest6 requires residual_bits=8.")
        ...
        self.stem                       = SignedINT4BasicViTStem(embed_dim, rounding, activation_range_momentum)
        self.local_block1               = SignedINT4LocalFeedForwardBlock(embed_dim, hidden_channels, rounding, activation_range_momentum)
        self.local_block2               = SignedINT4LocalFeedForwardBlock(embed_dim, hidden_channels, rounding, activation_range_momentum)
        self.attention_block1           = SignedINT4AttentionFeedForwardBlock(embed_dim, hidden_channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
        self.attention_block2           = SignedINT4AttentionFeedForwardBlock(embed_dim, hidden_channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
