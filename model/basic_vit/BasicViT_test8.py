"""BasicViT with FP4 weights, UINT4 ReLU activations, FP4 attention, and a shared INT8 residual stream."""
...
class Test8BasicViTStem(nn.Module):
    """Keep the image-facing convolution in INT8 and use UINT4 after the internal ReLU6 layers."""
    ...
    def forward(self, value: torch.Tensor, input_scale: torch.Tensor) -> torch.Tensor:
        ...
        value = self.conv2_depthwise(value, self.quantizer1.scale / 2.0)
        value = self.bn2_depthwise(value)
        value = self.relu2_depthwise(value)
        value = self.quantizer2_depthwise(value)

        value = self.conv2_pointwise(value, self.quantizer2_depthwise.scale / 2.0)
        value = self.bn2_pointwise(value)
        value = self.relu2_pointwise(value)
        return value


class Test8ConvolutionalFeedForward(nn.Module):
    """FP4-weight feed-forward network using UINT4 after every internal ReLU6."""
    ...
    def forward(self, value: torch.Tensor, input_scale: torch.Tensor) -> torch.Tensor:
        ...
        value = self.depthwise_conv(value, self.expand_quantizer.scale / 2.0)
        value = self.depthwise_bn(value)
        value = self.depthwise_relu(value)
        value = self.depthwise_quantizer(value)

        value = self.project_conv(value, self.depthwise_quantizer.scale / 2.0)
        value = self.project_bn(value)
        return value


class Test8LocalFeedForwardBlock(nn.Module):
    """Local feed-forward block connected to the globally shared INT8 residual stream."""

    def __init__(self, channels: int, hidden_channels: int, rounding: str, activation_range_momentum: float) -> None:
        super().__init__()
        self.ffn          = Test8ConvolutionalFeedForward(channels, hidden_channels, rounding, activation_range_momentum)
...
class Test8SimpleAttention(nn.Module):
    """FP4 Q/K/V, FP4 attention maps, FP4 matrix multiplications, and UINT4 after the output ReLU6."""
    ...
    def forward(self, value: torch.Tensor, input_scale: torch.Tensor) -> torch.Tensor:
        ...
        output    = self.project_conv(output, self.project_input_quantizer.scale / 2.0)
        output    = self.project_bn(output)
        return output


class Test8AttentionFeedForwardBlock(nn.Module):
    """FP4 attention and UINT4 feed-forward layers connected by one shared INT8 residual scale."""
    ...
    def __init__(self, channels: int, hidden_channels: int, num_heads: int, key_dim: int, value_dim: int, rounding: str, activation_range_momentum: float) -> None:
        super().__init__()
        self.attention              = Test8SimpleAttention(channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
...
class BasicViTTest8(nn.Module):
    """Test 8: FP4 weights, UINT4 ReLU activations, FP4 attention, and a globally shared INT8 residual scale."""
    ...
    def __init__(self, num_classes: int = 10, input_bits: int = 8, residual_bits: int = 8, rounding: str = "ties_away_from_zero", activation_range_momentum: float = 0.95, image_size: int = 32, embed_dim: int = 48, mlp_ratio: int = 2, num_heads: int = 2, key_dim: int = 12, value_dim: int = 24) -> None:
        super().__init__()
        if image_size != 32:
            raise ValueError("BasicViTTest8 is designed specifically for 32x32 images.")
        if input_bits != 8:
            raise ValueError("BasicViTTest8 requires input_bits=8.")
        if residual_bits != 8:
            raise ValueError("BasicViTTest8 requires residual_bits=8.")
        ...
        self.stem                       = Test8BasicViTStem(embed_dim, rounding, activation_range_momentum)
        self.local_block1               = Test8LocalFeedForwardBlock(embed_dim, hidden_channels, rounding, activation_range_momentum)
        self.local_block2               = Test8LocalFeedForwardBlock(embed_dim, hidden_channels, rounding, activation_range_momentum)
        self.attention_block1           = Test8AttentionFeedForwardBlock(embed_dim, hidden_channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
        self.attention_block2           = Test8AttentionFeedForwardBlock(embed_dim, hidden_channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
