"""CIFAR-10向けBasicViTのFP4 E2M1 QATモデル。"""
...
from utils.quantization import FP4MatMul, FP4Quantizer, IntegerQuantizer, PerTokenFP4NEQ, QuantConv2d, QuantLinear, QuantizedAvgPool2d, SharedIntegerQuantizer, SharedScaleResidualAdd


class FP4BasicViTStem(nn.Module):
    """最初のConvをINT8、それ以降の内部演算をFP4にするStem。"""
    ...
    def __init__(self, embed_dim: int, rounding: str, activation_range_momentum: float) -> None:
        ...
        self.quantizer1 = FP4Quantizer(rounding=rounding, range_momentum=activation_range_momentum)
        ...
        self.quantizer2_depthwise = FP4Quantizer(rounding=rounding, range_momentum=activation_range_momentum)
    ...
    def forward(self, value: torch.Tensor, input_scale: torch.Tensor) -> torch.Tensor:
        ...
        value = self.conv2_depthwise(value, self.quantizer1.scale / 4.0)
        value = self.bn2_depthwise(value)
        value = self.relu2_depthwise(value)
        value = self.quantizer2_depthwise(value)

        value = self.conv2_pointwise(value, self.quantizer2_depthwise.scale / 4.0)
        value = self.bn2_pointwise(value)
        value = self.relu2_pointwise(value)
        return value


class FP4ConvolutionalFeedForward(nn.Module):
    """FP4 E2M1重みと活性化を使用する畳み込みFFN。"""
    ...
    def __init__(self, channels: int, hidden_channels: int, rounding: str, activation_range_momentum: float) -> None:
        ...
        self.expand_quantizer = FP4Quantizer(rounding=rounding, range_momentum=activation_range_momentum)
        ...
        self.depthwise_quantizer = FP4Quantizer(rounding=rounding, range_momentum=activation_range_momentum)
    ...
    def forward(self, value: torch.Tensor, input_scale: torch.Tensor) -> torch.Tensor:
        ...
        value = self.depthwise_conv(value, self.expand_quantizer.scale / 4.0)
        value = self.depthwise_bn(value)
        value = self.depthwise_relu(value)
        value = self.depthwise_quantizer(value)

        value = self.project_conv(value, self.depthwise_quantizer.scale / 4.0)
        value = self.project_bn(value)
        return value


class FP4LocalFeedForwardBlock(nn.Module):
    """FP4 FFNと共有INT4 scale残差を持つローカルブロック。"""

    def __init__(self, channels: int, hidden_channels: int, rounding: str, activation_range_momentum: float) -> None:
        super().__init__()
        self.ffn = FP4ConvolutionalFeedForward(channels, hidden_channels, rounding, activation_range_momentum)
...
class FP4SimpleAttention(nn.Module):
    """FP4 NEQとFP4行列積を使用するSoftmaxなしSelf-Attention。"""
    ...
    def __init__(self, channels: int, num_heads: int, key_dim: int, value_dim: int, rounding: str, activation_range_momentum: float) -> None:
        ...
        self.project_input_quantizer = FP4Quantizer(rounding=rounding, range_momentum=activation_range_momentum)
    ...
    def forward(self, value: torch.Tensor, input_scale: torch.Tensor) -> torch.Tensor:
        ...
        output = self.project_conv(output, self.project_input_quantizer.scale / 4.0)
        output = self.project_bn(output)
        return output


class FP4AttentionFeedForwardBlock(nn.Module):
    """FP4 Attention・FFNを共有INT4 scale残差で連結するブロック。"""
    ...
    def __init__(self, channels: int, hidden_channels: int, num_heads: int, key_dim: int, value_dim: int, rounding: str, activation_range_momentum: float) -> None:
        super().__init__()
        self.attention = FP4SimpleAttention(channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
...
class BasicViTFP4(nn.Module):
    """FP4内部演算と6個の共有INT4 scale残差を持つBasicViT。"""
    ...
    def __init__(self, num_classes: int = 10, input_bits: int = 8, residual_bits: int = 4, rounding: str = "ties_away_from_zero", activation_range_momentum: float = 0.95, image_size: int = 32, embed_dim: int = 48, mlp_ratio: int = 2, num_heads: int = 2, key_dim: int = 12, value_dim: int = 24) -> None:
        super().__init__()
        if image_size != 32:
            raise ValueError("BasicViTFP4 is designed specifically for 32x32 images.")
        if input_bits != 8:
            raise ValueError("BasicViTFP4 requires input_bits=8.")
        if residual_bits != 4:
            raise ValueError("BasicViTFP4 requires residual_bits=4.")
        ...
        self.stem = FP4BasicViTStem(embed_dim, rounding, activation_range_momentum)
        self.local_block1 = FP4LocalFeedForwardBlock(embed_dim, hidden_channels, rounding, activation_range_momentum)
        self.local_block2 = FP4LocalFeedForwardBlock(embed_dim, hidden_channels, rounding, activation_range_momentum)
        self.attention_block1 = FP4AttentionFeedForwardBlock(embed_dim, hidden_channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
        self.attention_block2 = FP4AttentionFeedForwardBlock(embed_dim, hidden_channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
