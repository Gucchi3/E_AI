"""残差接続だけを独立INT8 scaleにしたBasicViTのUFP4テストモデル。"""
...
class Test1BasicViTStem(nn.Module):
...
class Test1ConvolutionalFeedForward(nn.Module):
...
class Test1LocalFeedForwardBlock(nn.Module):
    """このblock専用のINT8 scaleで残差加算するローカルブロック。"""

    def __init__(self, channels: int, hidden_channels: int, rounding: str, activation_range_momentum: float) -> None:
        super().__init__()
        self.ffn          = Test1ConvolutionalFeedForward(channels, hidden_channels, rounding, activation_range_momentum)
...
class Test1SimpleAttention(nn.Module):
...
class Test1AttentionFeedForwardBlock(nn.Module):
    """Attention残差とFFN残差が別々のINT8 scaleを持つブロック。"""
    ...
    def __init__(self, channels: int, hidden_channels: int, num_heads: int, key_dim: int, value_dim: int, rounding: str, activation_range_momentum: float) -> None:
        super().__init__()
        self.attention              = Test1SimpleAttention(channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
...
class BasicViTTest1(nn.Module):
    """UFP4内部活性化と6個の独立INT8残差scaleを持つアブレーションモデル。"""
    ...
    def __init__(self, num_classes: int = 10, input_bits: int = 8, residual_bits: int = 8, rounding: str = "ties_away_from_zero", activation_range_momentum: float = 0.95, image_size: int = 32, embed_dim: int = 48, mlp_ratio: int = 2, num_heads: int = 2, key_dim: int = 12, value_dim: int = 24) -> None:
        super().__init__()
        if image_size != 32:
            raise ValueError("BasicViTTest1 is designed specifically for 32x32 images.")
        if input_bits != 8:
            raise ValueError("BasicViTTest1 requires input_bits=8.")
        if residual_bits != 8:
            raise ValueError("BasicViTTest1 requires residual_bits=8.")
        ...
        self.stem                       = Test1BasicViTStem(embed_dim, rounding, activation_range_momentum)
        ...
        self.local_block1               = Test1LocalFeedForwardBlock(embed_dim, hidden_channels, rounding, activation_range_momentum)
        self.local_block2               = Test1LocalFeedForwardBlock(embed_dim, hidden_channels, rounding, activation_range_momentum)
        self.attention_block1           = Test1AttentionFeedForwardBlock(embed_dim, hidden_channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
        self.attention_block2           = Test1AttentionFeedForwardBlock(embed_dim, hidden_channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
