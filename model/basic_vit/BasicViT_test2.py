"""Attention経路だけをINT8にしたBasicViTのUFP4テストモデル。"""
...
class Test2ConvolutionalFeedForward(nn.Module):
...
class Test2LocalFeedForwardBlock(nn.Module):
    """UFP4 FFNをモデル全体で共有するINT4 scaleの残差へ加算する。"""

    def __init__(self, channels: int, hidden_channels: int, rounding: str, activation_range_momentum: float) -> None:
        super().__init__()
        self.ffn          = Test2ConvolutionalFeedForward(channels, hidden_channels, rounding, activation_range_momentum)
...
class Test2AttentionFeedForwardBlock(nn.Module):
    """INT8 AttentionとUFP4 FFNを共有INT4 scale残差で連結する。"""
    ...
    def __init__(self, channels: int, hidden_channels: int, num_heads: int, key_dim: int, value_dim: int, rounding: str, activation_range_momentum: float) -> None:
        super().__init__()
        self.attention              = Test2SimpleAttention(channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
...
class BasicViTTest2(nn.Module):
    """INT8 Attentionと6個の共有INT4 scale残差を持つUFP4モデル。"""
    ...
    def __init__(self, num_classes: int = 10, input_bits: int = 8, residual_bits: int = 4, rounding: str = "ties_away_from_zero", activation_range_momentum: float = 0.95, image_size: int = 32, embed_dim: int = 48, mlp_ratio: int = 2, num_heads: int = 2, key_dim: int = 12, value_dim: int = 24) -> None:
        super().__init__()
        if image_size != 32:
            raise ValueError("BasicViTTest2 is designed specifically for 32x32 images.")
        if input_bits != 8:
            raise ValueError("BasicViTTest2 requires input_bits=8.")
        if residual_bits != 4:
            raise ValueError("BasicViTTest2 requires residual_bits=4.")
        ...
        self.stem                       = Test2BasicViTStem(embed_dim, rounding, activation_range_momentum)
        self.local_block1               = Test2LocalFeedForwardBlock(embed_dim, hidden_channels, rounding, activation_range_momentum)
        self.local_block2               = Test2LocalFeedForwardBlock(embed_dim, hidden_channels, rounding, activation_range_momentum)
        self.attention_block1           = Test2AttentionFeedForwardBlock(embed_dim, hidden_channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
        self.attention_block2           = Test2AttentionFeedForwardBlock(embed_dim, hidden_channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
