"""BasicViT with FP4 weights, UFP4 ReLU activations, FP4 attention, and a shared INT4 residual stream."""
...
class Test9BasicViTStem(nn.Module):
    """Keep the image-facing convolution in INT8 and use UFP4 after the internal ReLU6 layers."""
...
class Test9ConvolutionalFeedForward(nn.Module):
    """FP4-weight feed-forward network using UFP4 after every internal ReLU6."""
...
class Test9LocalFeedForwardBlock(nn.Module):
    """Local feed-forward block connected to the globally shared INT4 residual stream."""

    def __init__(self, channels: int, hidden_channels: int, rounding: str, activation_range_momentum: float) -> None:
        super().__init__()
        self.ffn          = Test9ConvolutionalFeedForward(channels, hidden_channels, rounding, activation_range_momentum)
...
class Test9SimpleAttention(nn.Module):
    """FP4 Q/K/V, FP4 attention maps, FP4 matrix multiplications, and UFP4 after the output ReLU6."""
...
class Test9AttentionFeedForwardBlock(nn.Module):
    """FP4 attention and UFP4 feed-forward layers connected by one shared INT4 residual scale."""
    ...
    def __init__(self, channels: int, hidden_channels: int, num_heads: int, key_dim: int, value_dim: int, rounding: str, activation_range_momentum: float) -> None:
        super().__init__()
        self.attention              = Test9SimpleAttention(channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
...
class BasicViTTest9(nn.Module):
    """Test 9: FP4 weights, UFP4 ReLU activations, FP4 attention, and a globally shared INT4 residual scale."""
    ...
    def __init__(self, num_classes: int = 10, input_bits: int = 8, residual_bits: int = 4, rounding: str = "ties_away_from_zero", activation_range_momentum: float = 0.95, image_size: int = 32, embed_dim: int = 48, mlp_ratio: int = 2, num_heads: int = 2, key_dim: int = 12, value_dim: int = 24) -> None:
        super().__init__()
        if image_size != 32:
            raise ValueError("BasicViTTest9 is designed specifically for 32x32 images.")
        if input_bits != 8:
            raise ValueError("BasicViTTest9 requires input_bits=8.")
        if residual_bits != 4:
            raise ValueError("BasicViTTest9 requires residual_bits=4.")
        ...
        self.stem                       = Test9BasicViTStem(embed_dim, rounding, activation_range_momentum)
        self.local_block1               = Test9LocalFeedForwardBlock(embed_dim, hidden_channels, rounding, activation_range_momentum)
        self.local_block2               = Test9LocalFeedForwardBlock(embed_dim, hidden_channels, rounding, activation_range_momentum)
        self.attention_block1           = Test9AttentionFeedForwardBlock(embed_dim, hidden_channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
        self.attention_block2           = Test9AttentionFeedForwardBlock(embed_dim, hidden_channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
