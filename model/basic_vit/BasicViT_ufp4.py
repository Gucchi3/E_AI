"""CIFAR-10向けBasicViTのUFP4活性化QATモデル。"""
...
class UFP4BasicViTStem(nn.Module):
    """FP4重みとReLU6後のUFP4活性化を使用するStem。"""
...
class UFP4ConvolutionalFeedForward(nn.Module):
...
class UFP4LocalFeedForwardBlock(nn.Module):
    """UFP4 FFNと共有INT4 scale残差を持つローカルブロック。"""

    def __init__(self, channels: int, hidden_channels: int, rounding: str, activation_range_momentum: float) -> None:
        super().__init__()
        self.ffn = UFP4ConvolutionalFeedForward(channels, hidden_channels, rounding, activation_range_momentum)
...
class UFP4SimpleAttention(nn.Module):
    """符号付き部分にFP4、ReLU6後にUFP4を使用するSelf-Attention。"""
...
class UFP4AttentionFeedForwardBlock(nn.Module):
    """FP4/UFP4 Attention・FFNを共有INT4 scale残差で連結するブロック。"""
    ...
    def __init__(self, channels: int, hidden_channels: int, num_heads: int, key_dim: int, value_dim: int, rounding: str, activation_range_momentum: float) -> None:
        super().__init__()
        self.attention = UFP4SimpleAttention(channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
...
class BasicViTUFP4(nn.Module):
    """UFP4内部活性化と6個の共有INT4 scale残差を持つBasicViT。"""
    ...
    def __init__(self, num_classes: int = 10, input_bits: int = 8, residual_bits: int = 4, rounding: str = "ties_away_from_zero", activation_range_momentum: float = 0.95, image_size: int = 32, embed_dim: int = 48, mlp_ratio: int = 2, num_heads: int = 2, key_dim: int = 12, value_dim: int = 24) -> None:
        super().__init__()
        if image_size != 32:
            raise ValueError("BasicViTUFP4 is designed specifically for 32x32 images.")
        if input_bits != 8:
            raise ValueError("BasicViTUFP4 requires input_bits=8.")
        if residual_bits != 4:
            raise ValueError("BasicViTUFP4 requires residual_bits=4.")
        ...
        self.stem = UFP4BasicViTStem(embed_dim, rounding, activation_range_momentum)
        self.local_block1 = UFP4LocalFeedForwardBlock(embed_dim, hidden_channels, rounding, activation_range_momentum)
        self.local_block2 = UFP4LocalFeedForwardBlock(embed_dim, hidden_channels, rounding, activation_range_momentum)
        self.attention_block1 = UFP4AttentionFeedForwardBlock(embed_dim, hidden_channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
        self.attention_block2 = UFP4AttentionFeedForwardBlock(embed_dim, hidden_channels, num_heads, key_dim, value_dim, rounding, activation_range_momentum)
