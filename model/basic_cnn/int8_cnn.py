...
class Int8CNN(nn.Module):
    """重みと活性をINT8へFake Quantizationする分類モデル。"""
    ...
    def __init__(self, num_classes: int = 10, input_bits: int = 8, rounding: str = "ties_away_from_zero", activation_range_momentum: float = 0.95, image_size: int = 32) -> None:
        ...
        if input_bits != 8:
            raise ValueError("Int8CNN input_bits must be 8.")

        feature_size         = image_size // 8
        quantization         = BlockQuantization("integer", "integer", 8, 8)
        ...
        self.head            = QuantConvBlock(64, 64, stride=2, quantization=quantization, rounding=rounding, activation_range_momentum=activation_range_momentum)
