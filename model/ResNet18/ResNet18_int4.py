"""32x32のCIFAR-10入力に合わせたINT4 QAT ResNet-18。"""
...
class INT4BasicBlock(nn.Module):
    """両枝を同じINT4 scaleへ変換してから残差加算するBasicBlock。"""
    ...
    def __init__(self, in_channels: int, out_channels: int, stride: int, rounding: str, activation_range_momentum: float) -> None:
        ...
        self.conv1            = QuantConv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=1, bias=True, weight_bits=4, rounding=rounding, quantizer="integer")
        self.bn1              = nn.Identity()
        self.relu1            = nn.ReLU(inplace=False)
        self.conv1_quantizer  = IntegerQuantizer(bit_width=4, signed=False, rounding=rounding, range_momentum=activation_range_momentum)
        self.conv2            = QuantConv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=True, weight_bits=4, rounding=rounding, quantizer="integer")
        self.bn2              = nn.Identity()
        self.conv2_quantizer  = IntegerQuantizer(bit_width=4, signed=True, rounding=rounding, range_momentum=activation_range_momentum)
        self.residual_add     = QuantResidualAdd(bit_width=4, rounding=rounding, range_momentum=activation_range_momentum)
        self.relu2            = nn.ReLU(inplace=False)
        self.output_quantizer = IntegerQuantizer(bit_width=4, signed=False, rounding=rounding, range_momentum=activation_range_momentum)

        if use_projection:
            self.shortcut           = nn.Sequential(QuantConv2d(in_channels, out_channels, kernel_size=1, stride=stride, bias=True, weight_bits=4, rounding=rounding, quantizer="integer"), nn.Identity())
            self.shortcut_quantizer = IntegerQuantizer(bit_width=4, signed=True, rounding=rounding, range_momentum=activation_range_momentum)
        else:
    ...
    def output_scale(self) -> torch.Tensor:
        """次のblockへ渡すunsigned INT4 scaleを返す。"""
...
class ResNet18INT4(nn.Module):
    """Stem／分類層をINT8、Bodyと残差加算をINT4にしたResNet-18。"""
    ...
    def __init__(self, num_classes: int = 10, input_bits: int = 8, rounding: str = "ties_away_from_zero", activation_range_momentum: float = 0.95, image_size: int = 32) -> None:
        super().__init__()
        if image_size != 32:
            raise ValueError("ResNet18INT4 is adapted specifically for 32x32 CIFAR images.")
        if input_bits != 8:
            raise ValueError("ResNet18INT4 requires input_bits=8.")
        ...
        self.stem_quantizer   = IntegerQuantizer(bit_width=4, signed=False, rounding=rounding, range_momentum=activation_range_momentum)
        self.stage1           = nn.Sequential(INT4BasicBlock(64, 64, 1, rounding, activation_range_momentum), INT4BasicBlock(64, 64, 1, rounding, activation_range_momentum))
        self.stage2           = nn.Sequential(INT4BasicBlock(64, 128, 2, rounding, activation_range_momentum), INT4BasicBlock(128, 128, 1, rounding, activation_range_momentum))
        self.stage3           = nn.Sequential(INT4BasicBlock(128, 256, 2, rounding, activation_range_momentum), INT4BasicBlock(256, 256, 1, rounding, activation_range_momentum))
        self.stage4           = nn.Sequential(INT4BasicBlock(256, 512, 2, rounding, activation_range_momentum), INT4BasicBlock(512, 512, 1, rounding, activation_range_momentum))
