"""INT4 QAT ResNet-18 for 32x32 CIFAR-10 images."""
...
class INT4BasicBlock(nn.Module):
    """INT4 residual block whose post-ReLU activations use unsigned INT4."""
    ...
    def __init__(self, in_channels: int, out_channels: int, stride: int, rounding: str, activation_range_momentum: float) -> None:
        ...
        self.shortcut_quantizer  = IntegerQuantizer(bit_width=4, signed=True, rounding=rounding, range_momentum=activation_range_momentum) if stride != 1 or in_channels != out_channels else None
        self.shortcut            = nn.Identity() if stride == 1 and in_channels == out_channels else nn.Sequential(QuantConv2d(in_channels, out_channels, kernel_size=1, stride=stride, bias=True, weight_bits=4, rounding=rounding, quantizer="integer"), nn.Identity())
...
class ResNet18INT4(nn.Module):
    """CIFAR ResNet-18 with INT4 body weights and activations."""
    ...
    def _make_stage(self, out_channels: int, block_count: int, stride: int, rounding: str, activation_range_momentum: float) -> nn.Sequential:
        blocks = [INT4BasicBlock(self.in_channels, out_channels, stride, rounding, activation_range_momentum)]
        self.in_channels = out_channels
        blocks.extend(INT4BasicBlock(self.in_channels, out_channels, 1, rounding, activation_range_momentum) for _ in range(1, block_count))
    ...
    def _forward_stage(self, stage: nn.Sequential, value: torch.Tensor, input_scale: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        scale = input_scale
        for block in stage:
            if not isinstance(block, INT4BasicBlock):
                raise TypeError("INT4 ResNet stages must contain INT4BasicBlock modules.")
