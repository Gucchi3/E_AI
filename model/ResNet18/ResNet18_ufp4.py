"""32x32のCIFAR-10入力に合わせたFP4重み・UFP4活性化のQAT ResNet-18。"""
...
from utils.quantization import FP4Quantizer, IntegerQuantizer, QuantConv2d, QuantLinear, QuantResidualAdd, QuantizedAvgPool2d, UFP4Quantizer


class UFP4BasicBlock(nn.Module):
    """FP4重み、ReLU後UFP4、符号付き枝FP4、INT4残差加算のBasicBlock。"""
    ...
    def __init__(self, in_channels: int, out_channels: int, stride: int, rounding: str, activation_range_momentum: float) -> None:
        ...
        self.conv1_quantizer  = UFP4Quantizer(rounding=rounding, range_momentum=activation_range_momentum)
        ...
        self.output_quantizer = UFP4Quantizer(rounding=rounding, range_momentum=activation_range_momentum)
...
class ResNet18UFP4(nn.Module):
    """Stem／分類層をINT8、BodyをFP4/UFP4、残差加算をINT4にしたResNet-18。"""
    ...
    def __init__(self, num_classes: int = 10, input_bits: int = 8, rounding: str = "ties_away_from_zero", activation_range_momentum: float = 0.95, image_size: int = 32) -> None:
        super().__init__()
        if image_size != 32:
            raise ValueError("ResNet18UFP4 is adapted specifically for 32x32 CIFAR images.")
        if input_bits != 8:
            raise ValueError("ResNet18UFP4 requires input_bits=8.")
        ...
        self.stem_quantizer   = UFP4Quantizer(rounding=rounding, range_momentum=activation_range_momentum)
        self.stage1           = nn.Sequential(UFP4BasicBlock(64, 64, 1, rounding, activation_range_momentum), UFP4BasicBlock(64, 64, 1, rounding, activation_range_momentum))
        self.stage2           = nn.Sequential(UFP4BasicBlock(64, 128, 2, rounding, activation_range_momentum), UFP4BasicBlock(128, 128, 1, rounding, activation_range_momentum))
        self.stage3           = nn.Sequential(UFP4BasicBlock(128, 256, 2, rounding, activation_range_momentum), UFP4BasicBlock(256, 256, 1, rounding, activation_range_momentum))
        self.stage4           = nn.Sequential(UFP4BasicBlock(256, 512, 2, rounding, activation_range_momentum), UFP4BasicBlock(512, 512, 1, rounding, activation_range_momentum))
