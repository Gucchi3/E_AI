"""FP4-weight and UFP4-activation QAT ResNet-18 for CIFAR-10."""
...
from utils.quantization import FP4Quantizer, IntegerQuantizer, QuantConv2d, QuantLinear, UFP4Quantizer


class UFP4BasicBlock(nn.Module):
    """FP4-weight block using UFP4 after ReLU and FP4 on signed branches."""
...
class ResNet18UFP4(nn.Module):
    """CIFAR ResNet-18 with FP4 weights and UFP4 post-ReLU activations."""
    ...
    def _make_stage(self, out_channels: int, block_count: int, stride: int, rounding: str, activation_range_momentum: float) -> nn.Sequential:
        blocks = [UFP4BasicBlock(self.in_channels, out_channels, stride, rounding, activation_range_momentum)]
        self.in_channels = out_channels
        blocks.extend(UFP4BasicBlock(self.in_channels, out_channels, 1, rounding, activation_range_momentum) for _ in range(1, block_count))
    ...
    def _forward_stage(self, stage: nn.Sequential, value: torch.Tensor, input_scale: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        scale = input_scale
        for block in stage:
            if not isinstance(block, UFP4BasicBlock):
                raise TypeError("UFP4 ResNet stages must contain UFP4BasicBlock modules.")
