...
class QuantConvBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, stride: int, weight_bits: int, activation_bits: int, rounding: str, activation_range_momentum: float, kernel_size: int = 3, padding: int = 1) -> None:
        super().__init__()
        self.conv                 = QuantBNConv2d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding, bias=False, weight_bits=weight_bits, rounding=rounding, quantizer="integer")
...
class TinyQATCNN(nn.Module):
    ...
    def __init__(self, num_classes: int = 10, weight_bits: int = 8, activation_bits: int = 8, input_bits: int = 8, rounding: str = "ties_away_from_zero", activation_range_momentum: float = 0.95, image_size: int = 32) -> None:
        ...
        self.stem            = QuantConvBlock(3, 16, stride=1, weight_bits=weight_bits, activation_bits=activation_bits, rounding=rounding, activation_range_momentum=activation_range_momentum)
        self.stage1          = QuantConvBlock(16, 32, stride=2, weight_bits=weight_bits, activation_bits=activation_bits, rounding=rounding, activation_range_momentum=activation_range_momentum)
        self.stage2          = QuantConvBlock(32, 64, stride=2, weight_bits=weight_bits, activation_bits=activation_bits, rounding=rounding, activation_range_momentum=activation_range_momentum)
        self.head            = QuantConvBlock(64, 64, stride=1, weight_bits=weight_bits, activation_bits=activation_bits, rounding=rounding, activation_range_momentum=activation_range_momentum, kernel_size=final_kernel, padding=0)
