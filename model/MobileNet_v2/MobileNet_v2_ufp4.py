"""FP4-weight and UFP4-activation QAT MobileNetV2 for CIFAR-10."""
...
from utils.quantization import FP4Quantizer, IntegerQuantizer, QuantConv2d, QuantizerName, QuantLinear, UFP4Quantizer

from .MobileNet_v2_fp32 import _make_divisible


class UFP4ConvBNReLU6(nn.Sequential):
    """Explicitly configured convolution, ReLU6, and non-negative quantizer."""

    def __init__(self, in_channels: int, out_channels: int, weight_bits: int, weight_quantizer: QuantizerName, activation_quantizer: UFP4Quantizer | IntegerQuantizer, rounding: str, kernel_size: int = 3, stride: int = 1, groups: int = 1) -> None:
        padding = (kernel_size - 1) // 2
        super().__init__(QuantConv2d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding, groups=groups, bias=True, weight_bits=weight_bits, rounding=rounding, quantizer=weight_quantizer), nn.Identity(), nn.ReLU6(inplace=False))
...
class UFP4InvertedResidual(nn.Module):
    """FP4-weight block using UFP4 after ReLU6 and FP4 on signed outputs."""
    ...
    def __init__(self, in_channels: int, out_channels: int, stride: int, expand_ratio: int, rounding: str, activation_range_momentum: float) -> None:
        ...
        if stride not in {1, 2}:
            raise ValueError("UFP4InvertedResidual stride must be 1 or 2.")

        hidden_channels = int(round(in_channels * expand_ratio))
        layers: list[nn.Module] = []
        if expand_ratio != 1:
            layers.append(UFP4ConvBNReLU6(in_channels, hidden_channels, 4, "fp4", UFP4Quantizer(rounding=rounding, range_momentum=activation_range_momentum), rounding, kernel_size=1))
        layers.extend([UFP4ConvBNReLU6(hidden_channels, hidden_channels, 4, "fp4", UFP4Quantizer(rounding=rounding, range_momentum=activation_range_momentum), rounding, stride=stride, groups=hidden_channels), QuantConv2d(hidden_channels, out_channels, kernel_size=1, stride=1, padding=0, bias=True, weight_bits=4, rounding=rounding, quantizer="fp4"), nn.Identity()])
        self.block                = nn.Sequential(*layers)
        self.use_residual         = stride == 1 and in_channels == out_channels
        self.expand_ratio         = expand_ratio
        self.projection_quantizer = FP4Quantizer(rounding=rounding, range_momentum=activation_range_momentum)
        self.residual_quantizer   = FP4Quantizer(rounding=rounding, range_momentum=activation_range_momentum) if self.use_residual else None
...
class MobileNetV2UFP4(nn.Module):
    """CIFAR MobileNetV2 with FP4 weights and UFP4 post-ReLU6 activations."""
    ...
    def __init__(self, num_classes: int = 10, input_bits: int = 8, rounding: str = "ties_away_from_zero", activation_range_momentum: float = 0.95, image_size: int = 32, width_multiplier: float = 1.0, dropout: float = 0.2) -> None:
        super().__init__()
        if image_size != 32:
            raise ValueError("MobileNetV2UFP4 is adapted specifically for 32x32 CIFAR images.")
        if input_bits != 8:
            raise ValueError("MobileNetV2UFP4 requires input_bits=8.")
        ...
        features: list[nn.Module] = [UFP4ConvBNReLU6(3, input_channels, 8, "integer", UFP4Quantizer(rounding=rounding, range_momentum=activation_range_momentum), rounding, stride=1)]
        ...
        for expand_ratio, channels, repeats, first_stride in self.inverted_residual_setting:
            ...
            for block_index in range(repeats):
                ...
                features.append(UFP4InvertedResidual(input_channels, output_channels, stride, expand_ratio, rounding, activation_range_momentum))
                input_channels = output_channels

        features.append(UFP4ConvBNReLU6(input_channels, last_channels, 4, "fp4", IntegerQuantizer(bit_width=8, signed=False, rounding=rounding, range_momentum=activation_range_momentum), rounding, kernel_size=1))
    ...
    def forward(self, value: torch.Tensor) -> torch.Tensor:
        ...
        if not isinstance(stem, UFP4ConvBNReLU6):
            raise TypeError("The first MobileNetV2 feature must be UFP4ConvBNReLU6.")
        ...
        for index in range(1, len(self.features) - 1):
            ...
            if not isinstance(block, UFP4InvertedResidual):
                raise TypeError("MobileNetV2 middle features must be UFP4InvertedResidual modules.")
            value = block(value, scale)
            scale = block.output_scale

        head = self.features[-1]
        if not isinstance(head, UFP4ConvBNReLU6):
            raise TypeError("The final MobileNetV2 feature must be UFP4ConvBNReLU6.")
